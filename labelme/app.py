...
from . import utils
from .yolo11_settings import Yolo11SettingsDialog
from .yolo11_runner import run_yolo11
from .openpose_exporter import export_openpose_png, export_dataset_batch
from .yolo11_reinfer import reinfer_from_boxes
...
""",
        ...
        openpose_batch_action = action(
            self.tr("Exporter dataset OpenPose — dossier…"),
            lambda: export_dataset_batch(self),
            None, None,
            self.tr("Générer PNG + JSON OpenPose pour tout un dossier"),
        )
        reinfer_action = action(
            self.tr("Réinférer DWPose depuis les boxes…"),
            lambda: reinfer_from_boxes(self),
            None, None,
            self.tr("Relancer DWPose sur les boxes manuelles de l'image courante"),
        )
        openpose_export_action = action(
            self.tr("Exporter PNG OpenPose…"),
            lambda: export_openpose_png(self),
            None, None,
            self.tr("Exporter les skeletons cochés en PNG OpenPose"),
        )
        yolo11_settings_action = action(
            self.tr("Réglages YOLO11…"),
            lambda: Yolo11SettingsDialog(self).exec_(),
            None, None,
            self.tr("Configurer le pipeline YOLO11"),
        )
        yolo11_file_action = action(
            self.tr("YOLO11 — Fichier"),
            lambda: run_yolo11(self, pick_dir=False),
            None, "open",
            self.tr("Lancer YOLO11 sur un fichier"),
        )
        yolo11_dir_action = action(
            self.tr("YOLO11 — Dossier"),
            lambda: run_yolo11(self, pick_dir=True),
            None, "open",
            self.tr("Lancer YOLO11 sur un dossier"),
        )
        ...
        self.edit_menu_actions = (
            ...
            None,
            yolo11_settings_action,
            yolo11_file_action,
            yolo11_dir_action,
            None,
            reinfer_action,
            openpose_export_action,
            openpose_batch_action,
        )
    ...
    def _load_shape_dicts(self, shape_dicts: list[ShapeDict]) -> None:
        ...
        for shape_dict in shape_dicts:
            ...
            for point in shape_dict["points"]:
                if len(point) == 4:  # format skeleton [num_part, x, y, conf]
                    _, x, y, conf = point
                else:  # format normal [x, y]
                    x, y = point
            ...
            if shape_dict["shape_type"] == "skeleton":
                n = len(shape.points)
                sk_name = shape_dict.get("skeleton_name") or shape_dict.get("other_data", {}).get("skeleton_name")
                if sk_name:
                    # skeleton_name déjà écrit correctement à la génération
                    # (gère les groupes partiels : ex. openpose sans face = 66pts,
                    # ce qui ne tombe ni sur 17 ni sur 134 -> l'heuristique seule
                    # se trompait et retombait sur "coco_fullbody" par défaut)
                ...
                shape.other_data["skeleton_meta"] = shape_dict["points"]
    ...
    def saveLabels(self, filename):
        ...
        def format_shape(s):
            ...
            if s.shape_type == "skeleton" and "skeleton_meta" in data:
                meta = data.pop("skeleton_meta")
                skeleton_points = []
                for i, p in enumerate(s.points):
                    num_part, _, _, conf = meta[i]
                    skeleton_points.append([num_part, p.x(), p.y(), conf])
            else:
                skeleton_points = None
            data.update(
                dict(
                    label=s.label,
                    points=skeleton_points if skeleton_points is not None else [(p.x(), p.y()) for p in s.points],
                    group_id=s.group_id,
            ...
            # skeleton_name écrit explicitement à la racine du dict JSON
            # (other_data ne le contient pas, donc il disparaîtrait au cycle suivant)
