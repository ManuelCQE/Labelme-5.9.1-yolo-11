...
from labelme.shape import Shape, get_keypoint_name
...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._skeleton_submenu = None
        self._rubber_band_rect: QtCore.QRectF | None = None   # sélection cadre en cours
        self._rubber_band_start: QPointF | None = None
        self._rubber_band_vertices: list[tuple] = []  # [(shape, index), ...]
    ...
    def mouseMoveEvent(self, ev):
        ...
        # Rubber band sélection keypoints (Alt+drag)
        if Qt.LeftButton & ev.buttons() and ev.modifiers() == (Qt.ControlModifier | Qt.AltModifier):
            if self._rubber_band_start is not None:
                self._rubber_band_rect = QtCore.QRectF(
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            if index is not None:
                ...
                if shape.shape_type == "skeleton":
                    kp_name = get_keypoint_name(
                        getattr(shape, "skeleton_name", "human"), index
                    )
                    if kp_name:
                        tip = f"{index} — {kp_name}"
                        QtWidgets.QToolTip.showText(
                            self.mapToGlobal(ev.pos()), tip, self
                        )
                    else:
    ...
    def mousePressEvent(self, ev):
        ...
        if ev.button() == Qt.LeftButton and ev.modifiers() == (Qt.ControlModifier | Qt.AltModifier) and self.editing():
            # Début sélection cadre keypoints skeleton
            self._rubber_band_start = self.transformPos(ev.localPos())
            self._rubber_band_rect = QtCore.QRectF(self._rubber_band_start, self._rubber_band_start)
            self._rubber_band_vertices = []
            self.update()
            self._update_status()
            return
        if ev.button() == Qt.LeftButton:
            ...
            # Sauvegarder hVertex/hShape AVANT selectShapePoint qui peut les effacer
            self._rclick_hvertex = self.hVertex
            self._rclick_hshape  = self.hShape
            group_mode = int(ev.modifiers()) == Qt.ControlModifier
    ...
    def mouseReleaseEvent(self, ev):
        if ev.button() == Qt.RightButton:
            ...
            self._populate_skeleton_menu(menu)
            self.restoreCursor()
            ...
            if self._rubber_band_rect is not None and self._rubber_band_start is not None:
                # Fin rubber band — collecter les keypoints skeleton dans le rect
                self._rubber_band_vertices = []
                for shape in self.shapes:
                    if shape.shape_type != "skeleton" or not self.isVisible(shape):
                        continue
                    for i, pt in enumerate(shape.points):
                        if (pt.x() == 0 and pt.y() == 0):
                            continue
                        if self._rubber_band_rect.contains(pt):
                            self._rubber_band_vertices.append((shape, i))
                self._rubber_band_start = None
                self._rubber_band_rect = None
                self.update()
                if self._rubber_band_vertices:
                    # Afficher menu contextuel
                    menu = QtWidgets.QMenu(self)
                    n = len(self._rubber_band_vertices)
                    act = menu.addAction(f"Mettre à zéro {n} keypoint(s)")
                    act.triggered.connect(self._zero_rubber_band_vertices)
    ...
    def _populate_skeleton_menu(self, menu):
        # Remove previous dynamic skeleton submenu if present
        if hasattr(self, "_skeleton_submenu") and self._skeleton_submenu is not None:
            menu.removeAction(self._skeleton_submenu.menuAction())
            self._skeleton_submenu = None

        target_shape = None
        # Priorité : shape sous le curseur au clic droit
        rclick_hs = getattr(self, "_rclick_hshape", None)
        if rclick_hs is not None and rclick_hs.shape_type == "skeleton":
            target_shape = rclick_hs
        elif self.selectedShapes:
        ...
        # Supprimer le keypoint survolé (mettre à 0,0)
        # On utilise _rclick_hvertex sauvegardé au mousePressEvent
        rclick_hv = getattr(self, "_rclick_hvertex", None)
        rclick_hs = getattr(self, "_rclick_hshape", None)
        if rclick_hv is not None and rclick_hs is not None and rclick_hs.shape_type == "skeleton":
            kp_name = get_keypoint_name(
                getattr(rclick_hs, "skeleton_name", "human"), rclick_hv
            )
            label = f"Supprimer keypoint {rclick_hv}"
            if kp_name:
                label = f"Supprimer '{kp_name}'"
            del_act = menu.addAction(label)
            del_act.triggered.connect(
                functools.partial(self._zero_keypoint, rclick_hs, rclick_hv)
            )
            menu.addSeparator()

        missing = target_shape.missing_body_keypoints()
        if not missing:
            return

        submenu = QtWidgets.QMenu(self.tr("Add missing keypoint"), menu)
        for index, name in missing:
            label = name.replace("_", " ").capitalize()
            action = submenu.addAction(label)
            action.triggered.connect(
                functools.partial(self._add_missing_keypoint, target_shape, index)
            )
        menu.addMenu(submenu)
        self._skeleton_submenu = submenu

    def _add_missing_keypoint(self, shape, index):
        center = shape.body_bbox_center()
        shape.moveVertexBy(index, center - shape.points[index])
        self.storeShapes()
        self.shapeMoved.emit()
        self.update()

    def _zero_keypoint(self, shape, index):
        """Met un keypoint à (0,0) — équivalent suppression."""
        self.storeShapes()
        # Utiliser moveVertexBy comme _add_missing_keypoint
        offset = QPointF(0, 0) - shape.points[index]
        shape.moveVertexBy(index, offset)
        self.storeShapes()
        self.shapeMoved.emit()
        self.update()

    def _zero_rubber_band_vertices(self):
        """Met à (0,0) tous les keypoints skeleton sélectionnés par rubber band."""
        if not self._rubber_band_vertices:
            return
        self.storeShapes()
        for shape, index in self._rubber_band_vertices:
            offset = QPointF(0, 0) - shape.points[index]
            shape.moveVertexBy(index, offset)
        self._rubber_band_vertices = []
        self.storeShapes()
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:
        ...
        # Dessin rubber band sélection keypoints
        if self._rubber_band_rect is not None:
            p.save()
            p.scale(self.scale, self.scale)
            pen = QtGui.QPen(QtGui.QColor(0, 180, 255), 1, Qt.DashLine)
            p.setPen(pen)
            p.setBrush(QtGui.QColor(0, 180, 255, 40))
